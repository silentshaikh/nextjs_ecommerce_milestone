...
function Card({productimage,productname,productcategory,productid}:ClothList) {
  const {onProductDetail} = useClothContext();
  // const navigToDetail = useRouter();
  return (
    <div onClick={() => onProductDetail(productid,productcategory)}  className='cursor-pointer py-10 flex flex-col items-start max-[860px]:py-5 max-[360px]:py-2'>
      <Image
